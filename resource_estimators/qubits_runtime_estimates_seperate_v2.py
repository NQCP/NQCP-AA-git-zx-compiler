...
# rounds_per_attempt = 12 code cycles, counted from the fold-transversal f=5
# stage breakdown (App.: inject 1 + cultivation 6 + escape 5 = 12; equivalently
# 8 stabilizer rounds + 4 H_XY logical checks, per Sahay et al. Table A5).
#
# expected_attempts:
#   p = 1e-3 : discard rate δ_r = 80% → 5 expected attempts
#              (Sahay f=3 / LER≈2e-6 regime). 12 × 5 = 60.
...
#              Rounded up to 1.5 for safety margin → 12 × 1.5 = 18.
#              Neither Sahay nor Khan publish p=1e-4 numbers, so this is the best
#              analytical extrapolation available; the resulting cult-tAV curves
#              are still on the conservative side (true Poisson gives ~14 cycles).
...
def build_tav_parity_dist_rows(
    ...
    *,
    ...
    TAV_PARITY_FACTORY_PERIOD (=59) code cycles, no buffer bus.
