...
def process_sequence_to_hexagons(sequence_input):
    ...
    # KNOWN LIMITATION: only handles len(hexagons) == 2. If a qubit appears on
    # 3+ hexagons in the same sequence, no 'b' tags are written, so downstream
    # internal-bell-pair counts will undercount. bell_pairs_count.py's
    # diagnose_undertagged_sequences() flags this; empirically 0 cases on the
    # current fermi-hubbard data.
